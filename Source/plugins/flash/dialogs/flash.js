...
script: plugins/flash/dialogs/flash
...
(function()
{
			...
			switch ( attrDef.type )
			{
				...
				case ATTRTYPE_PARAM
					...
					else
					{
						...
						{
							var param = CKEDITOR.dom.element.createFromHtml( '<ckeparam></ckeparam>', objectNode.getDocument() );
	...
	CKEDITOR.dialog.add( 'flash', function( editor )
	{
		...
		var previewPreloader,
			...
			'<div id="cke_FlashPreviewLoader' + CKEDITOR.tools.getNextNumber() + '" style="displaynone"><div class="loading">&nbsp;</div></div>' +
		...
		return {
			...
			onShow  function()
			{
				...
				if ( fakeImage && fakeImage.data( 'cke-real-element-type' ) && fakeImage.data( 'cke-real-element-type' ) == 'flash' )
				{
					...
					if ( realElement.getName() == 'ckeobject' )
					{
					...
					else if ( realElement.getName() == 'ckeembed' )
			...
			onOk  function()
			{
				...
				if ( !this.fakeImage )
				{
					if ( makeObjectTag )
					{
						objectNode = CKEDITOR.dom.element.createFromHtml( '<ckeobject></ckeobject>', editor.document );
						var attributes = {
							classid  'clsidd27cdb6e-ae6d-11cf-96b8-444553540000',
					...
					if ( makeEmbedTag )
					{
						embedNode = CKEDITOR.dom.element.createFromHtml( '<ckeembed></ckeembed>', editor.document );
